...
def ensure_structure():
    ...
    try:
        ...
        summary.update_acell("A2",
            '=QUERY(Sales!A:H, "select B, sum(F), sum(H) where B is not null group by B label sum(F) \'Total Sales\', sum(H) \'Total Profit\'", 1)'
        )
...
def refund_sale(sku: str, size: str):
    ...
    return {"sku": sku, "size": size.upper(), "restored": 1, "sale_reversed": sale_price, "net_reversed": (sale_price - cost), "new_qty": cur + 1}
